...
function f9(){
    if(i9.value <= 32){
        out9.innerHTML = 1;
    }else if(i9.value > 32 && i9.value <= 43){
        out9.innerHTML = 2;
    }else if(i9.value > 43 && i9.value <= 64){
        out9.innerHTML = 3
    }else{
        out9.innerHTML = 0;
    }
...
//потому что значение по умолчанию string, а если сделать так: let v = +i130.value - будет number
...
function f15(){
    if(s153.value == "&&"){
        out15.innerHTML = s151.value + " или " + s152.value;
    }else{
        out15.innerHTML = s151.value + " и " + s152.value;
    }
}
