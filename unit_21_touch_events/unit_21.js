...
let a2 = document.querySelector('.div-2').addEventListener("touchstart", t2)
// ваше событие здесь!!!
...
function t3(event) {
    if(event.path[0].textContent == "3-1"){
        out3.innerHTML = 1;
    }else if(event.path[0].textContent == "3-2"){
        out3.innerHTML = 2;
    }
}
...
let b8 = document.querySelector('.div-8');
function t8() {
    let out = a8[Math.floor(Math.random()*a8.length)];
    b8.style.background = out;
}
b8.ontouchstart = t8;
// ваше событие здесь!!!
...
let a10 = document.querySelector('.div-10');
let w10 = 75;
function t10() {
    a10.style.width = w10 + "px";
    w10++;
}
a10.addEventListener('touchmove', t10);
// ваше событие здесь!!!
